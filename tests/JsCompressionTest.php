...
/**
 * JavaScript Compression and ASI (Automatic Semicolon Insertion) Test Suite
 *
...
function run_semicolon_tests(): bool
{
	$tests = [
		...
		[
			'name' =>
				'Tagged template then identifier (should insert semicolon before const)',
			'js' => "tag`tmpl`\nconst a=1",
			'mustContain' => [';const'],
		...
		[
			'name' =>
				'Plain template then identifier (should insert semicolon before let)',
			'js' => "`x\${1+2}`\nlet b=2",
			'mustContain' => [';let'],
			'mustNotContain' => ['` let'],
		],
		...
		[
			...
			'js' =>
				"async function r(){for await (const a of xs){await a()}}\nconst n=10",
			'mustContain' => ['};const'],
			'mustNotContain' => ['}const'],
		],
		[
