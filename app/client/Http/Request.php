...
interface Request {
   ...
   /**
    * Retrieves raw POST parameters natively populated by PHP.
    *
    * This method retrieves the value of a native $_POST parameter by key. If no key is specified, 
    * it returns all native POST parameters.
    *
   ...
   /**
    * Retrieves parsed form data from x-www-form-urlencoded payloads.
    *
    * This method parses and reads raw URL-encoded input streams, typically used for non-POST 
    * methods like PUT or PATCH.
    *
    * @param ?string $key The key of the form field.
    * @return mixed The form field value, or null if not set.
    */
   public function form (?string $key = null);

   /**
    * Retrieves parsed multipart/form-data payload for non-POST requests.
    *
    * Manually extracts form fields from raw multipart input streams when native $_POST 
    * is unavailable (e.g., PUT or PATCH requests).
    *
    * @param ?string $key The key of the multipart form field.
    * @return mixed The field value, or null if not set.
    */
   public function multipart (?string $key = null);

   /**
    * Retrieves all input data conditionally merged based on the request Content-Type.
    *
    * Automatically extracts and merges parameters from query strings, native $_POST, JSON bodies, 
    * URL-encoded streams, or multipart/form-data payloads.
    *
