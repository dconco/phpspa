...
abstract class HtmlCompressor
{
	...
	{
		if (self::$compressionLevel === static::LEVEL_NONE) {
			return $html;
		}
	...
	{
		if ($level === static::LEVEL_AUTO) {
			$level = self::detectOptimalLevel($html);
		}

		switch ($level) {
			case static::LEVEL_BASIC:
				return self::basicMinify($html);

			case static::LEVEL_AGGRESSIVE:
				return self::aggressiveMinify($html);

			case static::LEVEL_EXTREME:
	...
	private static function detectOptimalLevel(string $content): int
	{
		...
		if ($size < 1024) {
			// Less than 1KB
			return static::LEVEL_BASIC;
		} elseif ($size < 10240) {
			// Less than 10KB
			return static::LEVEL_AGGRESSIVE;
		} else {
			// 10KB or more
			return static::LEVEL_EXTREME;
		}
	}
}
