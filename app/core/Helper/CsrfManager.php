...
class CsrfManager
{
	...
	function __construct(string $name, string $sessionKey = '_csrf_tokens')
	{
		$this->name = $name;
	 ...
	 */
	function verify(bool $expireAfterUse = true): bool
	{
	 ...
	 * @return bool True if token is valid and not expired, false otherwise
	 */
	function verifyToken(string $token, bool $expireAfterUse = true): bool
	{
		if (empty($this->getSessionData()[$this->name])) {
			return false;
		}
