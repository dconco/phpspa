import {
	...
	StateObject,
	StateValueType
} from '../types/StateObjectTypes'
import {
	EventObject,
	EventPayloadMap,
	RuntimeConfig
} from '../types/RuntimeInterfaces'
import {
	clearPreloadedStylesForScope,
	preloadStylesFromContent,
	retainStylesheetLinks
} from '../utils/preloadStylesFromContent'
...
export class AppManager {
	...
	public static navigate(
		...
		const newUrl =
			url instanceof URL ? url : new URL(url, location.toString())
		...
		function processResponse(responseData: ComponentObject | string) {
			...
			if (component?.targetID) {
				RuntimeManager.currentRoutes[component.targetID] = {
					...
					defaultContent:
						RuntimeManager.currentRoutes[component.targetID]
							?.defaultContent ?? targetElement.innerHTML
				}
			}
				...
				if (targetInfo.exact === true && targetID !== component?.targetID) {
					...
					if (currentHTML) {
						try {
							morphdom(
								currentHTML,
								'<div>' + targetInfo.defaultContent + '</div>',
								{
									childrenOnly: true
								}
							)
			...
			const updateDOM = () =>
				new Promise(resolve => {
					const styleScopeKey =
						component?.targetID ||
						history.state?.targetID ||
						targetElement.id ||
						'__phpspa_body__'

					// --- Preload stylesheets in the new content ---
					const { element, ready } = preloadStylesFromContent(
						component.content,
						styleScopeKey
					)
	 ...
	 */
	public static on<K extends keyof EventObject>(
		event: K,
	...
	public static off<K extends keyof EventObject>(
		event: K,
		callback?: (payload: EventPayloadMap[K]) => void
	): void {
	...
	public static useCallback<T extends (...args: any[]) => any>(
		callback: T,
	...
	public static setState(
		key: string,
		value: StateValueType | ((previous: StateValueType) => StateValueType)
	): Promise<void> {
